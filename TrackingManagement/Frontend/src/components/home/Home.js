...
const Home = () => {
    ...
    const carsFilter = carsStatus.filter((item) => {
      for (const [key, value] of Object.entries(omitFilter)) {
        if (typeof item["car"][key] === "string") {
          if (!item["car"][key].toLowerCase().includes(value.toLowerCase()))
            return false
        } else {
  ...
  const checkRunningSegmentation = (segmentation) => {
    ...
    return now.isBetween(beginTime, endTime) && days.includes(now.isoWeekday())
  }
  ...
  const getCarInfo = (carId) => {
    const currentStatus = carsStatus.filter((item) => item.carId === carId)[0]
    const { segmentations, drivers, treasurers, atmTechnicans } = assets
    const hasSegmentation = segmentations.filter((item) => item.carId === carId)
    let carInfo = {}
    ...
    makeRequest({
      ...
      if (firstTime) {
        ...
        res.data.forEach((car) => {
          boundBuilder.extend(
            new google.maps.LatLng({ lat: car.gpsLat, lng: car.gpsLon })
          )
        })
  ...
  const handleEditRoute = (segmentation) => {
    ...
    if (checkSegmentationHasRouteEditted(segmentation)) {
      const editedSegmentationRoute = getEditedSegmentationRoute(segmentation)
  ...
  const onSelectSegmentation = (segmentation) => {
    ...
    const historyFilter = {
      ...
      beginTime: today
        .set(segmentationHandler.getUpdateTimeObject(segmentation.beginTime))
                    ...
                    {({ height, width }) => {
                      return (
                        <div
                          ...
                          className="custom-scroll-bar"
                        >
                          {displayCarsStt.map((item) => (
                            <Badge
                              count={
                                item.isSos ? (
                                  <p
                                    style={{
                                      backgroundColor: "#c300e4",
                                      color: "#ffffff",
                                      padding: "3px",
                                      borderRadius: "10px",
                                      fontWeight: "500",
                              ...
                              <div
                                style={{
                                  ...
                                  color: carConfig.status.filter(
                                    (sst) => sst.value === item.engineOn
                                  )[0].textColor,
                                  backgroundColor: carConfig.status.filter(
                                    (sst) => sst.value === item.engineOn
                                  )[0].color,
                                }}
          ...
          >
            {carsStatus.map((car) => (
              <>
                <Marker
                  key={car.id}
                  position={{ lat: car.gpsLat, lng: car.gpsLon }}
            ...
            {activeInfoWindow ? (
              <InfoWindow
                ...
                options={{ pixelOffset: new google.maps.Size(-8, -20) }}
                zIndex={102}
              >
                <CarBoxInfo
            ...
            {selectedCar ? (
              carsStatus
                ...
                .map((item) => (
                  <InfoBox
                    position={{ lat: item.gpsLat, lng: item.gpsLon }}
                    options={{
                      pixelOffset: new google.maps.Size(-23, -60),
                      closeBoxURL: "",
                      enableEventPropagation: true,
                    }}
                    zIndex={102}
                  ...
                  {({ height, width }) => (
                    ...
                    >
                      {assets.segmentations
                        .filter((item) => checkRunningSegmentation(item))
                        .map((item) => (
                          <div className="segmentation-item-wrapper">
                            ...
                            <div
                              ...
                              onClick={() => onSelectSegmentation(item)}
                            >
              ...
              <div style={{ flex: "1 1 auto" }}>
                <AutoSizer disableWidth>
                  {({ height, width }) => (
                    ...
                    >
                      {assets.segmentations
                        .filter((item) => checkPrepareSegmentation(item))
                        .map((item) => (
                          <div className="segmentation-item-wrapper">
                            <div className="edit-segmentation-btn">
                              <PlusCircleOutlined

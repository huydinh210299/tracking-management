...
const RFID = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return <>{record.isDistributed ? "Đã được gán" : "Chưa được gán"}</>
      },
      width: 150,
    },
    {
  ...
  return (
    <>
      ...
      <Drawer
        title={mode === RECORD_MODE.CREATE ? "Thêm thẻ mới" : "Cập nhật thẻ"}
        placement="right"
        onClose={onClose}
        visible={visible}
      >
        ...
        >
          ...
          <Form.Item label="isDistributed" name="isDistributed" noStyle>
            <Input type="hidden" />
          </Form.Item>
          <Form.Item
            label="Số thẻ"
          ...
          <Form.Item
            name="type"
            label="Loại thẻ"
            rules={[{ required: true, message: "Bạn phải chọn loại thẻ" }]}
          >
            <Select>
